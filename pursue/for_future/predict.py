...
'''
时间序列模型，growth代表模型，可选择linear、logistic两种模型进行训练，
缺省的情况下是linear模型，linear模型样本只需要时间ds列和随时间变化的自变量y
如果将growth设置为logistic则样本数据需要指定cap列，cap是自变量变化的最大值，
floor为可选列，标识自变量的最小值，floor的值不允许大于或等于cap值
'''
...
def predict_command(top,draw_forecast,draw_trend,save,hd):
    ...
    if top:
        ...
        logging.info('the forecast_set %s',forecast_set)
